...
results_dir = "./pbrt-results/vspgbuffer-vspg-spp8"
# path to store the post-processed results and the HTML viewer
viewer_output_dir = "./pbrt-viewers/vspgbuffer-vspg-spp8"

# Loading the test case descriptions from a file
testCases = TestCaseHelper.loadTestCases("pbrt-testcases/vspgbuffer")
testCaseDescription = TestCaseHelper.loadTestCaseDescription("pbrt-testcases/vspgbuffer")
...
for scene, scene_variants, resolution in scenes:
    for variant in scene_variants:
        ...
        for testCase in testCases:    
            pbrt.runTestCase(scene, variant, resolution, testCase, spp = 8, stats = True, usedGuidedGBuffer = False)
